...
if __name__ == '__main__':
    ...
    for ensemble in range(training_ensemble):
        if train:
            ...
            # model.evaluate(test_img, test_lbl_cat, batch_size=16)
    ...
    preds, wmh_zones, nawm_zones, gm_mask, lfb_img = postprocess.run(test_img, preds, lfb_img)
    for i in range(len(preds)):
        plot_prediction(test_img[i][:,:,1], test_lbl[i], wmh_zones[i], nawm_zones[i], gm_mask[i], lfb_img[i], i)
